...
export default function AuthPage() {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!otpMode) {
          ...
          // Verify OTP
          const res = await fetch("http://localhost:5005/api/verify-otp", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          const res = await fetch("http://localhost:5005/api/forgot-password", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const resendOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:5005/api/resend-otp", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
